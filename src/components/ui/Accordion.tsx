...
>(({ children, className, ...props }, ref) => {
    return (
        <AccordionPrimitive.Root
            ref={ref}
            className={cn(
                "border w-full rounded shadow-sm overflow-hidden relative transition-all",
                className
            )}
            {...props}
        >
    ...
    return (
        <AccordionPrimitive.Header className="flex !p-0 !m-0">
            <AccordionPrimitive.Trigger
                ref={ref}
                className={cn(
                    "relative z-[2] bg-secondary flex justify-between items-center py-4 px-6 w-full text-left text-base font-normal group",
                    "data-[state=closed]:hover:bg-border/50 transition-colors",
                    className
                )}
                {...props}
            >
